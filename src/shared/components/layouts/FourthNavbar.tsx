...
export default function StickyNavBar() {
  ...
  // Show once scrolled past the full header, hide only when back at/above it
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <>
      ...
      >
        <div className="flex justify-between items-center p-2 sm:p-4 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <form
              onSubmit={(e) => { e.preventDefault(); if (searchTerm.trim()) navigate(`/Shop?q=${encodeURIComponent(searchTerm.trim())}`); }}
              className="flex items-center bg-white/20 rounded-full px-2 sm:px-3 py-1"
            >
              <input
                type="text"
                value={searchTerm}
      ...
      {/* Rendered outside <nav> to avoid fixed stacking context clipping */}
